...
class LocoEnv:
    ...
    def _randomize_link_length(self, env_ids):
        min_scale, max_scale = self.env_cfg['link_length_range']
        link_names = self.env_cfg['link_names_to_randomize']
        ...
        if not link_indices:
            print("Warning: No matching links found for randomization:", link_names)
            return
        
        scale_factors = gs.rand((len(env_ids), len(link_indices)), dtype=float) \
                        * (max_scale - min_scale) + min_scale
        
        self.rigid_solver.set_links_geometry_scale(
            scale_factors, [idx + self.robot.link_start for idx in link_indices], env_ids
        )

...
def get_cfgs():
    env_cfg = {
        ...
        'links_to_keep': ['fl_foot', 'fr_foot', 'rl_foot', 'rr_foot','chassis'],
        'num_actions': 8,
        'num_dofs': 8,
    ...
    reward_cfg = {
        ...
        'reward_scales': {
            ...
            'base_height': -100.0,         # Reduce from -40.0 to -10.0 to allow height variation
            'torques': -0.0002,           # Unchanged   -0.0002,
...
def main():
    ...
    if args.debug:
        args.vis = True
        args.offline = True
        args.num_envs = min(args.num_envs, 100)
    ...
    rewards = []
    def log_rewards():
        mean_reward = env.rew_buf.mean().item()
        rewards.append(mean_reward)
        plt.plot(rewards)
        plt.xlabel('Step')
        plt.ylabel('Mean Reward')
        plt.savefig(f'{log_dir}/reward_plot.png')
        plt.close()
        return mean_reward
    ...
    for step in range(1000):
        # Replace with correct method from OnPolicyRunner
        actions = runner.actor_critic.act(obs)[0].detach()  # Common in RSL-RL
        obs, rewards, dones, infos = env.step(actions.to(env.device))
        if step % 10 == 0:
            mean_reward = log_rewards()
            print(f"Step {step}, Mean Reward: {mean_reward}")
        if dones.any():
            obs, _ = env.reset()

    if args.record_video:
        env.stop_recording(save_path=f'{log_dir}/training_video.mp4')

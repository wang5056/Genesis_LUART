...
from locomotion_env_height import LocoEnv
...
def get_cfgs():
    env_cfg = {
        'urdf_path': '/home/tt/Genesis/genesis/assets/urdf/tt_feb/urdf/mingsong_feb_urdf.urdf',
        ...
        'termination_if_height_lower_than': 0.01,


        # base pose
        'base_init_pos': [6.0, 6.0, 0.35],
        ...
        # 'terrain_cfg': {
        #     'n_subterrains': (3, 3),  # Number of subterrains in (x,y) directions
        #     'horizontal_scale': 0.1,   # Horizontal resolution in meters per grid cell
        #     'vertical_scale': 0.05,    # Vertical resolution in meters per height step
        #     'subterrain_size': (8.0, 8.0),  # Size of each subterrain in meters (x,y)
        #     'subterrain_types': [
        ...
        'terrain_cfg': {
            ...
            # 'subterrain_types': [
            #     ['flat_terrain', 'sloped_terrain'],
            #     ['stairs_terrain', 'wave_terrain']
            # ],
            'subterrain_types':[['flat_terrain']]
            # Keep randomization ranges conservative
    ...
    obs_cfg = {
        'num_obs': 9 + 1 + 3 * env_cfg['num_dofs'], # ths + 1 is for commands_height
        'num_history_obs': 1,
        'obs_noise': {
            'ang_vel': 0.1,
            'gravity': 0.02,
        ...
        'num_priv_obs': 12 + 1 + 4 * env_cfg['num_dofs'], # the +1 is for commands
    }
    reward_cfg = {
        'tracking_sigma': 0.25,
...
def main():
    ...
    parser.add_argument('-e', '--exp_name', type=str, default='newturtle-angyaw-height-tuichang1-show-viewer-flat')

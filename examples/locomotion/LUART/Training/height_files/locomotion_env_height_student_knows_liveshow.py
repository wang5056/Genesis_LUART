...
class LocoEnv:
    def __init__(
        ...
        assert self.action_latency in [0, 0.04]
        ...
        self.scene = gs.Scene(
            ...
            viewer_options=gs.options.ViewerOptions(
                max_FPS=60,
                camera_pos=(10.0, 10.0, 5.0),
                camera_lookat=(0.0, 0.0, 0.5),
                camera_fov=60,
            ),
            vis_options=gs.options.VisOptions(
                n_rendered_envs=min(self.num_envs, 100),
                env_grid_size=(10, 10),
                env_spacing=(2.0, 2.0),
            ),
        ...
        if self.env_cfg['use_terrain']:
            ...
            self.scene.add_entity(
                gs.morphs.URDF(file='urdf/plane/plane_light.urdf', fixed=True),
            )
        ...
        # Initialize URDF paths for randomization
        self._env_urdf_paths = {}
        env_ids = torch.arange(self.num_envs, dtype=gs.tc_int, device=self.device)
        self._randomize_link_length(env_ids)

        # Add robots per environment
        self.robots = {}
        for env_id in env_ids.tolist():
            urdf_path = self._env_urdf_paths.get(env_id, self.env_cfg['urdf_path'])
            ...
            self.robots[env_id] = robot
        self.robot = self.robots[0]  # For compatibility
    ...
    def _set_camera(self):
        self._cameras = []
        # Overview camera
        self._cameras.append(self.scene.add_camera(
        ...
        # Follow camera
        if self.num_envs > 1:
            self._follow_camera = self.scene.add_camera(
            ...
            self._cameras.append(self._follow_camera)

    def _render_headless(self):
        if self._recording and len(self._recorded_frames) < 150:
            if hasattr(self, '_follow_camera'):
                env_id = np.random.randint(self.num_envs)
                robot_pos = self.base_pos[env_id].cpu().numpy()
                self._follow_camera.set_pose(
                    pos=robot_pos + np.array([0.0, -2.0, 1.0]),
                    lookat=robot_pos + np.array([0.0, 0.0, 0.5])
                )
            for cam in self._cameras:
                frame, _, _, _ = cam.render()
                self._recorded_frames.append(frame)

    def _draw_debug_vis(self):
        self.scene.clear_debug_objects()
        for env_id in range(min(self.num_envs, 100)):
            foot_poss = self.foot_positions[env_id].reshape(-1, 3).cpu().numpy()
            ...
            com = self.com[env_id].cpu().numpy()
            reward = self._reward_tracking_lin_vel()[env_id].item()
            color = (1 - reward, reward, 0, 0.7)  # Red to green
    ...
    def stop_recording(self, save_path=None):
        if save_path:
            frames = [np.uint8(frame) for frame in self._recorded_frames]
            h, w = frames[0].shape[:2]
            writer = cv2.VideoWriter(save_path, cv2.VideoWriter_fourcc(*'mp4v'), 60, (w, h))
            for frame in frames:
                writer.write(frame)
            writer.release()
    ...
    def _init_buffers(self):
        ...
        self.commands_scale = torch.tensor(
            [
                self.obs_scales['lin_vel'], #2
                self.obs_scales['lin_vel'], #2
                self.obs_scales['ang_vel'], #0.25
                self.obs_scales['height_info'], #1
            ],
            device=self.device,
            dtype=gs.tc_float,
        )
    ...
    def _update_buffers(self):
        ...
        # Also compute the gravity vector in the body frame.
        self.projected_gravity = gs_transform_by_quat(self.global_gravity, inv_base_quat)

        # Update the degrees-of-freedom (dofs) positions and velocities.
        ...
        # Update the center-of-mass (COM) and foot-related states.
        ...
        ''' Update base position and orientation from the global quat, pos'''
        # self.base_pos[:] = self.robot.get_pos()
        # self.base_quat[:] = self.robot.get_quat()
        
        # # Compute Euler angles directly from the current base quaternion.
        # # (This gives you the actual current yaw, pitch, roll.)
        # self.base_euler = gs_quat2euler(self.base_quat)
        
        # # Use the full inverse of the current base quaternion to transform the velocities.
        # # This correctly brings the world-frame velocities into the robot's body frame.
        # inv_base_quat = gs_inv_quat(self.base_quat)
    ...
    def get_observations(self):
        ...
        result = (self.obs_history_buf, extras)
        print(f"LocoEnv.get_observations returning: {len(result)} values - obs_shape={result[0].shape}, extras={result[1]}")
        return result
    ...
    def post_physics_step(self):
        ...
        if self.command_type == 'heading':
            ...
            self.commands[:, 2] = torch.clip(
                0.5 * wrap_to_pi(self.commands[:, 4] - heading), -1.0, 1.0
            )
    ...
    def _prepare_obs_noise(self):
        self.obs_noise[:, :3] = self.obs_cfg['obs_noise']['ang_vel']    # 3
        self.obs_noise[:, 3:6] = self.obs_cfg['obs_noise']['gravity']  # 3
        self.obs_noise[:, 6:10] = 0.0                                  # 4 (commands, typically no noise)
        self.obs_noise[:, 10:18] = self.obs_cfg['obs_noise']['dof_pos'] # 8
        self.obs_noise[:, 18:26] = self.obs_cfg['obs_noise']['dof_vel'] # 8
        self.obs_noise[:, 26:34] = 0.0                                 # 8 (actions, typically no noise)
        self.obs_noise[:, 34] = self.obs_cfg['obs_noise']['height_info']    # 1 (height noise)
    ...
    def _resample_commands(self, envs_idx):
        ...
        self.commands[envs_idx, :2] *= (
            torch.norm(self.commands[envs_idx, :2], dim=1) > 0.1  #0.2
        ).unsqueeze(1)
        ...
        if self.command_type == 'heading':
            ...
            self.commands[envs_idx, 2] *= torch.abs(self.commands[envs_idx, 2]) > 0.1
    ...
    def step(self, actions):
        ...
        infos = self.extras.copy()
        if self.num_privileged_obs is not None:
            infos['observations'] = {'critic': self.privileged_obs_buf}
    ...
    def _randomize_rigids(self, env_ids=None):
        ...
        if self.env_cfg['randomize_link_length']:  # New condition
            self._randomize_link_length(env_ids)
    ...
    def _randomize_link_length(self, env_ids):
        ...
        if not link_names:
            print("Warning: No links specified for randomization:", link_names)
            return
        
        # Generate scale factors for all environments
        scale_factors = gs.rand((len(env_ids), len(link_names)), dtype=float) \
                        * (max_scale - min_scale) + min_scale
        
        # Load the base URDF
        urdf_path = self.env_cfg.get('urdf_path', '/home/tt/Genesis/genesis/assets/urdf/ttt_chassis/urdf/ttt_chassis.urdf')  # Adjust based on your config
        if not os.path.exists(urdf_path):
            raise FileNotFoundError(f"URDF file not found: {urdf_path}")
        
        tree = ET.parse(urdf_path)
        
        # Create a directory for environment-specific URDFs
        tmp_dir = "/tmp/genesis_urdfs"
        os.makedirs(tmp_dir, exist_ok=True)
        
        # Store URDF paths for each environment
        self._env_urdf_paths = {env_id: None for env_id in env_ids}
        
        for env_idx, env_id in enumerate(env_ids):
            env_tree = deepcopy(tree)
            env_root = env_tree.getroot()
            
            for link_idx, link_name in enumerate(link_names):
                # Find the link in the URDF
                for link in env_root.findall('link'):
                    if link.get('name') == link_name:
                        # Modify collision geometry (cylinder length)
                        for collision in link.findall('collision'):
                            for geometry in collision.findall('geometry'):
                                for cylinder in geometry.findall('cylinder'):
                                    length = float(cylinder.get('length'))  # e.g., 0.23
                                    new_length = length * scale_factors[env_idx, link_idx]
                                    cylinder.set('length', str(new_length))
                        
                        # Optionally adjust inertial properties (mass ~ length^3, inertia ~ length^5)
                        for inertial in link.findall('inertial'):
                            for mass in inertial.findall('mass'):
                                mass_value = float(mass.get('value'))
                                mass.set('value', str(mass_value * scale_factors[env_idx, link_idx] ** 3))
                            for inertia in inertial.findall('inertia'):
                                for attr in ['ixx', 'iyy', 'izz']:
                                    value = float(inertia.get(attr))
                                    inertia.set(attr, str(value * scale_factors[env_idx, link_idx] ** 5))
                        break
            
            # Save modified URDF
            env_urdf_path = os.path.join(tmp_dir, f"ttt_chassis_env{env_id}.urdf")
            env_tree.write(env_urdf_path)
            self._env_urdf_paths[env_id] = env_urdf_path

...
class LocoEnv:
    def __init__(
        ...
        self.robot = self.scene.add_entity(
            gs.morphs.URDF(
                ...
                merge_fixed_links=True,  # this helps merge links with fixed joints in between
                links_to_keep=self.env_cfg['links_to_keep'],  # the feet should not be merged
        ...
        # self._floating_camera.render(rgb=True)  # new line added    
    ...
    def _randomize_link_length(self, env_ids):
        ...
        # Generate a single scale factor per environment for leg length
        scale_factors = gs.rand((len(env_ids), 1), dtype=float) * (max_scale - min_scale) + min_scale
        
        # Generate COM offset factors per environment (±10% of leg length)
        com_offset_range = 0.25  # ±10% of leg length (adjustable)
        com_offsets = gs.rand((len(env_ids), 1), dtype=float) * 2 * com_offset_range - com_offset_range  # [-0.1, 0.1]
        
        # Load the base URDF
        urdf_path = self.env_cfg.get('urdf_path', '/home/tt/Genesis/genesis/assets/urdf/ttt_chassis/urdf/ttt_chassis.urdf')
        ...
        for env_idx, env_id in enumerate(env_ids):
            ...
            scale = scale_factors[env_idx, 0]  # Single scale for leg length
            com_offset = com_offsets[env_idx, 0]  # COM offset factor
            base_length = 0.23  # Base cylinder length (adjust based on ttt_chassis.urdf)
            scaled_length = base_length * scale  # New leg length
            com_shift = com_offset * scaled_length  # COM offset in meters (±10% of scaled length)
            ...
            for link_name in link_names:
                ...
                for link in env_root.findall('link'):
                    if link.get('name') == link_name:
                        found = True
                        # print(f"Env {env_id}: Scaling {link_name} by {scale}, COM offset {com_shift}m")
